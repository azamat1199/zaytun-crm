...
const useForeignLanguage = () => {
    const { getWord } = useKeyTranslation();
    const { data, ...rest } = useQuery({
        queryKey: ["foreign-language-type"],
        queryFn: () =>
            getDegreeType({
                type: "KNOWING_FOREIGN_LANGUAGE",
            }),
        select: (response) => response.data?.result?.data,
    });

    const optionsLanguage = Array.isArray(data)
...
export default useForeignLanguage;
